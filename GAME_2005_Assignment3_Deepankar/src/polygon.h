...
{
public:
	polygon(int NumberOfVeritces, int Radius)
	{
		NoVertices = NumberOfVeritces;
		radius = Radius;
...
private:
	void GenerateVertices()
	{
		...
		if(NoVertices >= 3)
		{
			float const Constangle = (360/NoVertices) * Util::Deg2Rad;
			float tempAngle = 0;
	...
	void GenerateLines()
	{
		...
		lines->push(Line((*Vertices)[NoVertices-1], (*Vertices)[0], color));
	}
