...
void playscene2::start()
{
	...
	m_pBrick = new Brick();
	m_pNumberVertices = new int(16);
	m_GenPolygon = new polygon(*m_pNumberVertices,50);
	m_borders = new Borders();
	CollisionPoints = new UnorderedArray<CPoints>(3);
	CollisionPointsDraw = new UnorderedArray<CPoints>(3);
	ENERGYLOSS = new float(0.9f);
	drawCollisionPoints = new bool(false);
}

void playscene2::BounceWithCollisionLines(CPoints collisionPoint, bool isBrick)
{
	if(isBrick)
	{
		m_GenPolygon->FixPosition(collisionPoint.Line, m_pBrick);
		m_GenPolygon->Bounce(*ENERGYLOSS, collisionPoint.Line);
	}else
	{
...
bool playscene2::checkCollision(UnorderedArray<Line>& lines1, UnorderedArray<Line>& lines2) //Line by Line
{
...
glm::vec2 playscene2::CollisionPoint(Line line1, Line line2)
{
...
void playscene2::GUI_Function() const
{
	...
	ImGui::Begin("Control center", NULL, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_MenuBar);
	
	ImGui::Separator();

	ImGui::SliderFloat("Energy Loss", ENERGYLOSS, 0.05f,1.f);
	...
	ImGui::Separator();
	ImGui::LabelText("ROTATION ANGLE SPEED [rad/s]",std::to_string(m_GenPolygon->getAngleSpeed()).c_str());
	ImGui::LabelText("Speed of the polygon", std::to_string(Util::magnitude(m_GenPolygon->GetVelocity())).c_str());
	ImGui::Separator();
