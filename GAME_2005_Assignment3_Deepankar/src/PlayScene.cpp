...
void PlayScene::GUI_Function() const
{
	...
	ImGui::Text("CONTROL THE DELAY AND BULLET POOL");
	
	ImGui::Separator();
	if (ImGui::InputInt("NUMBER OF BULLETS", m_paramsImGui->newSizePool))
	{
		m_pBulletPool->SetNewSize(*m_paramsImGui->newSizePool);
	}


	ImGui::DragFloat("ADD DELAY FOR SPAWN", m_paramsImGui->timeToRespawn,0.01f,0.01f, 3.f);
